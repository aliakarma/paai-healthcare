...
def compute_reward(state: np.ndarray, action: int, vital: dict,
                   config: dict, action_mask: np.ndarray) -> float:
    ...
    bp_r   = 1.0 if sbp_lo <= sbp <= sbp_hi else -abs(sbp - np.clip(sbp, sbp_lo, sbp_hi)) / 20.0
    tir_r  = 1.0 if glc_lo <= glc <= glc_hi else -abs(glc - np.clip(glc, glc_lo, glc_hi)) / 50.0
