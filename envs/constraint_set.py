...
class ConstraintSet:
    ...
    def is_feasible(self, action: int, vitals: dict,
                     adherence_med: float) -> bool:
        if action == 4:
            return (self.registry.should_escalate(vitals) or
                    self.registry.should_watch(vitals))
        if action == 1 and adherence_med > 0.92:
            return False
        return True

    def violation_rate(self, actions: np.ndarray, vitals_list: list,
                        adherence_list: list) -> float:
        violations = sum(
