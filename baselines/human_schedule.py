...
_REVIEW_MINUTE = 0
_SBP_THRESHOLD = 160.0  # mmHg — clinician escalates if SBP > this at review
...
def _score_row(row: dict, rng: np.random.Generator) -> tuple[int, float]:
    ...
    score = float(np.clip(base + rng.normal(0.0, 0.05), 0.0, 1.0))
    return action, score
...
def _compute_latency(
    ...
    MAX_LATENCY_SEC = 24.0 * 3600.0  # 24 hours
    ...
    for pid, grp in vitals_df.groupby("patient_id"):
        ...
        for event_t in event_times:
            ...
            if escalated_t is not None:
                latency_sec = (escalated_t - event_t) * 60.0
...
def evaluate(cohort_dir: str) -> dict:
    ...
    for _, row in tqdm(
        vitals_df.iterrows(),
        total=len(vitals_df),
        desc="Evaluating Human-schedule",
        leave=False,
    ):
