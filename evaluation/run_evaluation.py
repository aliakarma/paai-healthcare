    ...
    for m in METHODS:
        ...
        if m != "aghealth":
            pv = bonferroni_correct(
                delong_test(r["roc_auc"], results["aghealth"]["roc_auc"]),
                n_tests=3)
            pv_str = f"< {pv:.3f}"
        else:
            pv_str = "—"
...
def run_mimic(mimic_dir: str, model_path: str, output_dir: str):
    ...
    print(f"  ROC AUC        : {res.get('roc_auc', 0):.3f} "
...
def main():
    ...
    parser.add_argument("--mode",        choices=["synthetic", "mimic", "all"],
                         default="synthetic")
    ...
    if not Path(f"{args.cohort_dir}/vitals_longitudinal.csv").exists():
        print(f"Cohort not found. Run first:\n"
