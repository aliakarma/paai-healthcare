...
def plot_adherence(results: dict = None, save_path: str = None):
    ...
    ax.plot(weeks, [v * 100 + rng.normal(0, 0.5) for v in aghealth_adherence],
            "o-", color="#1a7ab5", lw=2.5, ms=6, label="AgHealth+")
    ax.plot(weeks, [v * 100 + rng.normal(0, 0.4) for v in rules_adherence],
            "s--", color="#5ba85f", lw=1.8, ms=5, label="Rules-only")

    ax.set_xlabel("Week", fontsize=11)
    ax.set_ylabel("Adherence (%)", fontsize=11)
    ax.set_title("Composite Nutritional Adherence over 8 Weeks",
                  fontsize=12, fontweight="bold")
    ax.legend(fontsize=9)
    ax.set_ylim([40, 70]); ax.set_xticks(weeks)
