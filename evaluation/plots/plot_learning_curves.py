...
def plot_learning_curves(tensorboard_dir: str = None, save_path: str = None):
    ...
    ep_reward = (
        -2.0 + 3.5 * (1 - np.exp(-steps / 400_000)) + rng.normal(0, 0.15, len(steps))
    )
    # Constraint violation rate: drops below 5% threshold quickly
    viol_rate = 0.25 * np.exp(-steps / 200_000) + rng.normal(0, 0.01, len(steps)).clip(
        0
    )
    ...
    ax2.axhline(y=5.0, color="red", ls="--", lw=1.5, label="Threshold (5%)")
    ...
    fig.suptitle(
        "RL Training Convergence — MaskablePPO", fontsize=12, fontweight="bold"
    )
    plt.tight_layout()
    if save_path:
