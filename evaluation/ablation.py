...
class _PassthroughConstraintFilter:
    ...
    def filter(self, proposed_action: int, vitals: dict,
               adherence_med: float, active_policies: dict):
...
def _evaluate_variant(
    ...
    for pid, grp in tqdm(vitals_df.groupby("patient_id"),
        ...
        for _, row in grp.iterrows():
            ...
            if use_rl and model is not None:
                raw = np.array([
                    vd.get("sbp",          130.), vd.get("dbp",           82.),
                    vd.get("glucose_mgdl", 110.), vd.get("heart_rate",    72.),
                    vd.get("spo2",          97.5),
                ], dtype=np.float32)
                ...
                rolling_slope = vitals_z - (rolling_mean - 1.)
                adherence     = np.array([
                    vd.get("adherence_med",       0.7),
                    vd.get("adherence_diet",      0.5),
                    vd.get("adherence_lifestyle", 0.6)], dtype=np.float32)
                t_h    = (t / 60.) % 24.
                context = np.array([t_h / 24., (t_h // 24. % 7.) / 7.,
                                    float(abs(t_h % 4. - 2.) < 0.5),
                                    vd.get("adherence_lifestyle", 0.6)],
                                   dtype=np.float32)
                obs = np.clip(np.concatenate([
                    vitals_z, rolling_mean - 1., rolling_slope,
                    adherence, context, np.zeros(3, dtype=np.float32)]),
                    -10., 10.).astype(np.float32)[np.newaxis, :]
                ...
                if is_maskable:
                    act, _ = model.predict(obs, action_masks=mask[np.newaxis, :],
                                           deterministic=True)
                else:
            ...
            score = float(np.clip(
                base_score + np.random.default_rng(t).normal(0, 0.03), 0., 1.))
            ...
            if act == 4 and first_abnormal_t is not None and not escalated:
                latency_sec = (t - first_abnormal_t) * 60.
    ...
    for _ in range(1000):
        ...
        boot_precs.append(float(precision_score(yt, predicted_esc,
                                                 zero_division=0)))

    lat_arr = np.array(latencies, dtype=float) if latencies else np.array([3600.])
