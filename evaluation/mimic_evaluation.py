...
def evaluate_mimic(vitals_path: str, events_path: str) -> dict:
    ...
    def anomaly_score(row) -> float:
        ...
        if sbp >= bp["systolic_emergency"]: score = 0.95
        elif sbp >= bp["systolic_urgent"]:  score = max(score, 0.70)
        if glucose <= glc["hypoglycemia_severe"]: score = 0.95
        elif glucose <= glc["hypoglycemia_mild"]: score = max(score, 0.65)
        if spo2 <= spo2_thr["emergency"]:         score = 0.95
        elif spo2 <= spo2_thr["alert"]:           score = max(score, 0.60)
    ...
    if len(np.unique(y_true)) < 2:
        return {"error": "Not enough positive examples in MIMIC subset",
    ...
    return {
        ...
        "roc_auc":      float(roc),
        "auc_ci":       (float(np.percentile(boot, 2.5)),
