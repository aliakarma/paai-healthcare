...
class ConflictResolver:
    ...
    def resolve(self, tasks: list[dict], vitals: dict,
                 running_sodium_mg: float = 0.0) -> list[dict]:
        """
    ...
    def _check_task(self, task: dict, vitals: dict,
                     running_sodium_mg: float) -> tuple[bool, str]:
        t = task.get("type", "")
        ...
        if t == "dietary_modification":
            ...
            if running_sodium_mg + sodium > cap:
                return True, f"sodium_budget_exceeded ({running_sodium_mg + sodium:.0f}mg)"
        ...
        if t == "dietary_modification":
            if (task.get("potassium_rich") and
                    self.registry.rules.get("renal_adjustments")):
