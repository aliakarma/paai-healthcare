...
def test_orchestrator_step_returns_required_keys(orchestrator):
    ...
    vitals = {"sbp": 130, "dbp": 82, "glucose_mgdl": 140,
               "heart_rate": 72, "spo2": 97,
               "adherence_med": 0.75, "adherence_diet": 0.55}
    result = orchestrator.step(42, vitals, _patient_state())

    for key in ("patient_id", "goals", "events", "tasks_resolved",
...
def test_orchestrator_emergency_triggered_on_high_bp(orchestrator):
    ...
    vitals = {"sbp": 192, "dbp": 112, "glucose_mgdl": 120,
               "heart_rate": 80, "spo2": 97,
               "adherence_med": 0.8, "adherence_diet": 0.6}
    result = orchestrator.step(42, vitals, _patient_state())

    event_types = [ev["type"] for ev in result["events"]]
    assert "acute_vital_exceedance" in event_types, (
        f"Expected acute_vital_exceedance in {event_types}")

    assert "emergency_escalation" in result["goals"], (
...
def test_orchestrator_audit_logged(orchestrator, tmp_audit):
    ...
    vitals = {"sbp": 125, "dbp": 80, "glucose_mgdl": 110,
               "heart_rate": 70, "spo2": 98,
               "adherence_med": 0.9, "adherence_diet": 0.7}
    orchestrator.step(42, vitals, _patient_state())
    assert tmp_audit.entry_count > initial_count, (
