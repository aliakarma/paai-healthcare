...
def rolling_volatility(series: np.ndarray, window: int) -> np.ndarray:
    ...
    -------
    np.ndarray
        Rolling volatility as float32 array.
    """
    s = pd.Series(series)
    return (
...
def rolling_zscore(series: np.ndarray, window: int) -> np.ndarray:
    ...
    -------
    np.ndarray
        Rolling z-scores as float32 array.
    """
    s = pd.Series(series)
...
def extract_all_features(
    channel_name: str, values: np.ndarray, windows: list[int] | None = None
) -> dict[str, np.ndarray]:
    ...
    ----------
    channel_name : str
        Name of the physiological channel (e.g., 'sbp').
    values : np.ndarray
        Time series values for the channel.
    windows : list[int], optional
        Rolling window sizes in timesteps (default [1, 3, 12]).
    
    Returns
    -------
    dict[str, np.ndarray]
        Dictionary mapping feature names to float32 arrays.
    """
    if windows is None:
        windows = [1, 3, 12]
